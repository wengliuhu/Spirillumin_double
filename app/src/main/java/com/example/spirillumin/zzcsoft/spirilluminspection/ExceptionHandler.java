...
public class ExceptionHandler implements UncaughtExceptionHandler {
	...
	public void collectDeviceInfo(Context ctx) {
		...
		infos.put("VERSION", android.os.Build.VERSION.RELEASE);
	}
	...
	private String getFilePath() {
		...
		if (isSDCardExist && isRootDirExist) {
			file_dir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/spirilluminspection/crashlog/";
		} else {
			// MyApplication.getInstance().getFilesDir()返回的路劲为/data/data/PACKAGE_NAME/files，其中的包就是我们建立的主Activity所在的包
			file_dir = BaseApplication.getInstance().getFilesDir().getAbsolutePath() + "/spirilluminspection/crashlog/";
		}
	...
	private String saveCatchInfo2File(Throwable ex) {
		...
		sb.append(result);
		// 发送异常信息到服务器
		sendCrashLogService(sb.toString()); //
		try {
	...
	private void sendCrashLogService(String message) {
		// HttpPostTask mAuthTask = new HttpPostTask();
		// ArrayMap<String, String> map = new ArrayMap<String, String>();
		// map.put("userId", BaseApplication.getInstance().getUserId());
		// map.put("errorMessage", message);
		//
		// mAuthTask.post("http://192.168.1.105/testServer/errorBase.do", map,
		// null);
	}
}